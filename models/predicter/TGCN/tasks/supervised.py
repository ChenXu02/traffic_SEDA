...
class SupervisedForecastTask(pl.LightningModule):
    def __init__(
        ...
        feat_max_val: list =None,
        feat_max_val2: list=None,
        **kwargs
    ):
    ...
    def validation_step(self, batch, batch_idx):
        predictions, y = self.shared_step(batch, batch_idx)
        predictions = predictions * self.feat_max_val2[1]+self.feat_max_val2[0]
        y = y * self.feat_max_val[1]+self.feat_max_val[0]
        loss = self.loss(predictions, y)
