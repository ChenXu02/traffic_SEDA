...
def BATF_Gibbs(dense_tensor, sparse_tensor, vector, factor, burn_iter, gibbs_iter,filename):
    ...
    for it in range(burn_iter + gibbs_iter):
        ...
        if it==50:
            ...
            tensor_hat_p=tensor_hat.reshape(826,-1)
            np.save('Imputation_'+filename,tensor_hat_p)
            print('saved!')
...
for j in {'cm','pm'}:
    for i in range(1,10):
        ...
        print(filename)
        dense_tensor=np.load('LSTM-GL-ReMF/CD_cm_0.npy')[:,0,:].reshape(826,-1,144)
        sparse_tensor=np.load('LSTM-GL-ReMF/'+filename)[:,0,:].reshape(826,-1,144)
        ...
        burn_iter = 200
        gibbs_iter = 200
        BATF_Gibbs(dense_tensor, sparse_tensor, vector, factor, burn_iter, gibbs_iter,filename)
        end = time.time()
