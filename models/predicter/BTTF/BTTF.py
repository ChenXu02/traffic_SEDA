...
def BTTF_forecast(filename,dense_tensor, sparse_tensor, pred_step, multi_step,
    ...
    for t in range(max_count):
        if t == 0:
            ...
            tensor, U, V, X_new, A, Sigma, tau = BTTF(dense_tensor[:, :, : start_time],
                                                      sparse_tensor[:, :, : start_time], init, rank, time_lags,
    ...
    testPred_mape=compute_mape(small_dense_tensor[pos], tensor_hat[pos])
    testPred_rmse=compute_rmse(small_dense_tensor[pos], tensor_hat[pos])
    testPred_mae = compute_mae(small_dense_tensor[pos], tensor_hat[pos])
    print(small_dense_tensor[pos], tensor_hat[pos],'pos')
    print('Prediction MAPE: {:.6}'.format(testPred_mape))
    print('Prediction RMSE: {:.6}'.format(testPred_rmse))
    print('Prediction MAE: {:.6}'.format(testPred_mae))
    print()
    return tensor_hat
...
for j in {'cm'}:
    for i in range(1,10):
        filename='CD_{}_0.{}.npy' .format(j,i)
        print(filename)
        dense_tensor=np.load('../LSTM-GL-ReMF/CD_pm_0.npy')[:,0,:].reshape(826,1,-1)#826
        sparse_tensor=np.load('../LSTM-GL-ReMF/'+filename)[:,0,:].reshape(826,1,-1)
        ...
        for multi_step in [1]:
            ...
            tensor_hat = BTTF_forecast(filename,dense_tensor, sparse_tensor, pred_step, multi_step,
