...
class DCRNNSupervisor:
    def __init__(self, filename,adj_mx, **kwargs):
        self._kwargs = kwargs
        ...
        # data set
        #print(self._file_kwargs,**self._data_kwargs)
        self._data = utils.load_dataset(self._file,**self._data_kwargs)
    ...
    def save_model(self, epoch):
        ...
        config['epoch'] = epoch
        torch.save(config, 'models/'+self._file+'.tar')
        self._logger.info("Saved model at {}".format(epoch))
        return 'models/'+self._file+'.tar'

    def load_model(self):
        self._setup_graph()
        checkpoint = torch.load('models/'+self._file+'.tar', map_location='cpu')
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            if dataset=='test':
                small_dense_tensor=np.array(y_truths_scaled)
                tensor_hat=np.array(y_preds_scaled)
                pos = np.where(small_dense_tensor != 0)
                testPred_mape = self.compute_mape(small_dense_tensor[pos], tensor_hat[pos])
                testPred_rmse = self.compute_rmse(small_dense_tensor[pos], tensor_hat[pos])
                testPred_mae = self.compute_mae(small_dense_tensor[pos], tensor_hat[pos])
